...
def faceshifter_batch(src_id_embedding: torch.tensor, 
                      target: torch.tensor,
                      source: torch.tensor,
    ...
    if bs > 1:
        src_id_embedding = torch.cat([src_id_embedding]*bs)  ##source_emb를 batch_size만큼 복사를 해서 B C H W로 브로드캐스팅한다
        source = torch.cat([source]*bs)
    
    with torch.no_grad():
        Y_st, _, _ = G(target, source, src_id_embedding) 
